...
-- a visible mismatch instead of a silent null, so the reconciliation cannot pass by
-- accident when a whole event date is absent from one side.
...
with agg as (
    ...
    from {{ ref('agg_transactions_channel_daily') }}
    group by 1
